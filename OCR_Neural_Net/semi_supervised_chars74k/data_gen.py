...
"""
    Dataset names of their folders from http://www.ee.surrey.ac.uk/CVSSP/demos/chars74k/
...
class Feature_Extraction ():
    def __init__(self,minibatch_size,real_size,proj_directory="./" ):
    ...
    def read_data(self):
        ...
        if self.train_x[1] is not None  :
            ...
            save_test_data(self.test_x,self.test_y,self.proj_directory) #Save test data for test check in the future
...
def load_data(iter,proj_directory="./",minibatch_size = 128):
    """
        This module loads the data and returns the sets to create the NN .
    """
    image = Feature_Extraction(minibatch_size)
    image.read_data(proj_directory)

    return {"prefix/input_real": image.train_x}
